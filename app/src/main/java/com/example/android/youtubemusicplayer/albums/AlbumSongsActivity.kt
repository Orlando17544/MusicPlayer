...
class AlbumSongsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        topAppBar.title = album?.name;

        val application = requireNotNull(this).application;

        val dataSource = MusicDatabase.getInstance(application).musicDatabaseDao;

        val viewModelFactory = AlbumSongsViewModelFactory(dataSource, application);

        viewModel = ViewModelProvider(this, viewModelFactory)
            .get(AlbumSongsViewModel::class.java);

        val adapter = SongsAdapter();

        album?.albumId?.let {
            viewModel.getSongsWithAlbumAndArtistByAlbumId(it).observe(this, Observer {
                it?.let {
        ...
        MusicPlayer?.currentSongWithAlbumAndArtist?.let { currentSongWithAlbumAndArtist ->
            album?.albumId?.let {
                viewModel.getSongsWithAlbumAndArtistByAlbumId(it).observe(this, Observer { songsWithAlbumAndArtist ->
